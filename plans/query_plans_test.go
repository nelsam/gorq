...
func (suite *QueryPlanTestSuite) getQueryPlanFor(value interface{}) *QueryPlan {
	...
	q := Query(suite.Map, suite.Map, val, nil, false, nil)
...
func (suite *QueryLanguageTestSuite) TearDownTest() {
	...
	case dialects.SqliteDialect:
		...
		_, err = Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).Delete()
	default:
		err = Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).Truncate()
	}
...
func (suite *QueryLanguageTestSuite) insertInvoices() {
	...
	for _, inv := range testInvoices {
		err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_TransientField() {
	q := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_Update() {
	...
	count, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).
	...
	invTest, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).
		Where().
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_SelectSimple() {
	invTest, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).Select()
	if suite.NoError(err) {
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_CountSimple() {
	count, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).Count()
	if suite.NoError(err) {
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_OrderBy_ASC() {
	invTest, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).OrderBy(&suite.Ref.Updated, "asc").Select()
	if suite.NoError(err) {
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_OrderBy_DESC() {
	invTest, err := Query(suite.Map, suite.Map, suite.Ref, nil, false, nil).OrderBy(&suite.Ref.Updated, "DESC").Select()
	if suite.NoError(err) {
		previous := invTest[0].(*OverriddenInvoice).Updated
		for _, result := range invTest {
			inv := result.(*OverriddenInvoice)
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_FakeEmbed() {
	...
	results, err := Query(suite.Map, suite.Map, fakeEmbed, nil, false, nil).
		Where().
...
func (suite *QueryLanguageTestSuite) TestQueryLanguage_WhereClauseIn() {
	...
	count, err := Query(suite.Map, suite.Map, ref, nil, false, nil).
		Where().
		In(&ref.Id, ids...).
		Count()
	if suite.NoError(err) {
